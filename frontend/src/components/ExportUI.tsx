...
type Props = {
  refetch: () => void
  setQueryParams: (params: { start_date?: string; end_date?: string; match_string?: string }) => void
  data: unknown
  isLoading: boolean
  error: unknown
}

const ExportUI = ({ refetch, setQueryParams, data, isLoading, error }: Props) => {
  ...
  const isReady = range?.[0] && range?.[1] && match.trim() !== ""
  ...
  return (
    <Space direction="vertical" size="middle" style={{ width: "100%" }}>
      <RangePicker
        value={range}
        onChange={(val) =>
          setRange(val && val.length === 2 ? (val as [dayjs.Dayjs, dayjs.Dayjs]) : undefined)
        }
      />
      <Input
